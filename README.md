...
- 사람인 공개 검색 결과
- 자소설닷컴 검색 결과
- 점핏 공개 검색 API와 상세 API (신입 지원 가능 공고만 요청)
- 잡플래닛 공개 채용 검색 API (모집 유형에 신입이 포함된 공고만 유지)
- 잡코리아 직무별 공고 보드 (robots.txt가 허용하는 /recruit/joblist를
  AI/ML 직무 코드와 신입 필터로 요청. 금지된 키워드 검색은 사용하지 않음)
- 원티드 전체 공개 채용 목록 (공통 키워드 필터 적용)
- 기업 채용 목록 페이지의 JSON-LD 또는 지정한 공고 상세 URL 패턴
  (SK·현대자동차·현대오토에버·카카오·쿠팡·토스·라인·우아한형제들.
  삼성전자·LG·네이버는 목록이 비공개 API로만 렌더링되어 기본 비활성화)

각 사이트의 `robots.txt`와 공개 범위를 확인하며, 로그인·CAPTCHA 등 접근 제어를 우회하지 않습니다.
Google Sheets의 `Sources` 시트로 소스를 관리하는 경우에는 `wanted` 행을 추가하고 `enabled`를 켜야 합니다.
